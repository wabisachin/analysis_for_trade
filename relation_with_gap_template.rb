#ギャップアップ(ギャップダウン)の順張りエントリー期待値(割合ベースで指定)

require 'csv'
require './module/analysis_tool'
# CSVデータをtable型のデータに変換
data_name =  "CSV/7267_2011_2019.CSV"
...
count_sell_win =  0
trade_list =  []
puts 'ギャップ値幅を何パーセント以上に設定しますか'
gap_width =  gets.to_i/100.to_f

#繰り返し処理
(count-1).times do |i|
    data_today =  table[i+1]
    data_yesterday =  table[i]
    if gap_up_for_rate?(data_today,data_yesterday,gap_width)
        # ギャップアップ時は買いエントリー
        ...
        puts data_today
        puts total_profit
    elsif gap_down_for_rate?(data_today,data_yesterday,gap_width)
        # ギャップダウン時は売りエントリー
...
puts "設定エントリー条件: ギャップ幅: #{gap_width*100}%"
puts "全営業日: #{count}"
